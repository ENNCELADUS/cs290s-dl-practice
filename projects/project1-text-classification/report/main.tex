...
\begin{enumerate}
  ...
  \item \textbf{Pooling.}  The \texttt{[CLS]} token representation from the
    final encoder layer is used as the sentence vector.
  \item \textbf{Projection MLP.}  A linear projection from 768 to a 512-dimensional
...
The encoder is fully frozen; only the projection MLP and classification head
are trained, giving approximately 394{,}754 trainable parameters out of
$\sim$102\,M total.  Using a frozen encoder makes the model fast to train and
avoids overfitting on the small task-specific dataset, but limits the
backbone's ability to adapt its representations to the sentiment domain.
...
surprising given the task context.  The frozen baseline extracts only the \texttt{[CLS]} token and projects it to a
